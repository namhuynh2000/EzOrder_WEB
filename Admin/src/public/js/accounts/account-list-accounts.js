...
deleteAccountForm = document.forms['delete-account-form'];
...
$('#detailAccountDialog').on('show.bs.modal', function (event) {
    ...
    $.getJSON (`api/accounts/${AccountId}/detail`, function(result) {
        result.createdAt = new Date(result.createdAt).toLocaleString("VN");
        currentAccount = result;
         var accountHtml = templateAccountModal({account: result});
        document.querySelector("#detailAccountDialog .modal-body").innerHTML = accountHtml;

    })
})

$('#confirm-delete-product-modal').on('show.bs.modal', function (event) {
    button = $(event.relatedTarget); // Button that triggered the modal
    document.querySelector("#confirm-delete-product-modal .modal-body").innerHTML = `Bạn có chắc muốn xóa tài khoản "${currentAccount.username}"`;
    // $.getJSON (`api/accounts/${AccountId}/detail`, function(result) {
    //     console.log(result);

    //     currentAccoutn = result;
    //      var accountHtml = templateAccountModal({account: result});
    //     document.querySelector("#detailAccountDialog .modal-body").innerHTML = accountHtml;
...
btnDeleteAccount.onclick = function() {
    deleteAccountForm.action = '/accounts/' + currentAccount._id + '/' + currentAccount.role + '?_method=DELETE';
